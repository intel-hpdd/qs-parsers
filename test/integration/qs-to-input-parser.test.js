...
const assignHostname = qsToInput.assign(
...
const statusQsToInputParser = fp.flow(tokenizer, statusParser, x => x.result);
  ...
  const inputOutput = {
    ...
    a: new Error(
      'Expected one of __contains, __startswith, __endswith, __in, =, ' +
        '__gte, __lte, __gt, __lt got end of string'
    ),
    ...
    'a__in=b&&': new Error(
      'Expected one of value, hostname got & at character 8'
    ),
